...
class InterparkOrder(SiteNewOrder):
    ...
    def logOn(self):                                    # 로그인 성공시 True, 실패시 False 리턴
        ...
        header2 = dict(header, **{"Upgrade-Insecure-Requests":"1"})         # 두번째 post 요청에 필요한 header
        r = self.s.post("https://ipss.interpark.com/member/login.do?_method=login",
                   data=payload, headers=header)  # 로그인
        r = self.s.post("https://ipss.interpark.com/member/login.do", data=payload2, headers=header2) # 두번째 post까지 있어야 쿠키값을 제대로 받음
    ...
    def getNewOrderNum(self):                           # 신규 주문 목록 개수 리턴
        ...
        endate = str(enday).replace('-', '')            # '-' 없는 str값으로 변환, 날짜 값을 get 요청에 같이 넣어줌으로 오늘을 기점으로 2주전까지의 목록을 불러옴
        r = self.s.get("https://ipss.interpark.com/delivery/ProOrderCheckList.do?_method=list&_style=grid&_search=false&nd=1485218711647&rows=50&page=1&sidx=&sord=asc&sc.dateTp=2&sc.strDt="+stdate+"&sc.strHour=00&sc.endDt="+endate+"&sc.endHour=23&sc.searchTp=&sc.searchNm=&flag=ko&sc.page=1&sc.row=50")
        result = r.json()                               # json 파싱
        return len(result['bs'])                        # bs 키값 안의 리스트 길이를 구하고 리턴
